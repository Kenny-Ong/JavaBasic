package JavaClass.Class2;

public class VariableCreation {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		//1 way to create a variable
		byte b=10; //create a variable and assign value
				
		//2 way to create a variable
		int i; //create a variable
		i=100; //assign value
		
		//we need to create a variable only once
		//then we can use as many times w want/need during the program
		
		//3 way
		//create a variable or declare a variable (space)
		int num1, num2, num3;
		
		num1=100; // assign value or initialize variable
		num2=200; 
		num3=300;
		
		System.out.println(num2); //200
		
		//Can I change a value of variable??
		
		num2=2000; //reassign the value
		
		System.out.println(num2); //2000 printout last value if there is a change
		
		//1 variable can hold only 1 value at 1 time/per line
		
		
		//boolean is either true or false
		boolean boo=true;
		boo=false;
		boo=true;
		
		
	}

}
